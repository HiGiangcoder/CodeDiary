...
void make_test(int iTest) {
    int n, q;
    if (Sub(0, 30)) {
        n = random(1, getRandom(1, 1e3, -NTEST, NTEST, NTEST));
        q = getRandom(min((int)1e3, n * 10), 1e3, -NTEST, NTEST, NTEST);
    
        vector<int> numran;
        int len = random(1, q / n * 20 + 7);

        REP(rep, len) numran.push_back(random(2, 1e2));
        int num = numran[random(0, len - 1)];

        cout << n << ' ' << q << '\n';
        for (int i = 1; i <= n; i ++) cout << random(1, 10) * num << " \n"[i == n];
        for (int i = 1; i <= q; i ++) {
            cout << random(1, n) << ' ' << random(1, 10) * numran[random(0, len - 1)] << "\n";
        }
    }
    else {
        n = random(1, getRandom(1, 1e5, -NTEST, NTEST, NTEST));
        q = getRandom(min((int)1e5, n * 10), 1e5, -NTEST, NTEST, NTEST);
    
        vector<int> numran;
        int len = random(1, min(3, q / n * 20 + 7));

        REP(rep, len) numran.push_back(random(2, 2e4));
        int num = numran[random(0, len - 1)];
